...
int main()
{
    ...
    std::cout << dix.size() << " :dix.size() == diy.size(): " << diy.size() << "\n1 == vsy.size(): " << vsy.size() << "\n3 == vsx.size(): " << vsx.size()
              << "\nRuby == vsy[0]: " << vsy[0] << "\n7 == ciy.front(): " << ciy.front() << "\n4 == cix.size(): " << cix.size() << "\n1 == std::count(diy.begin(), diy.end(), max - 1): " << std::count(diy.begin(), diy.end(), max - 1) << "\n1 == std::count(vsx.begin(), vsx.end(), C++): " << std::count(vsx.begin(), vsx.end(), "C++") << std::endl;
